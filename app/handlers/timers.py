...
class Timers:
    ...
    async def plan_timer(self, message: Message, state: FSMContext):
        ...
        await message.answer("Введіть час у форматі <b>години</b>:<b>хвилини</b>(00:00).\nЩоб таймер повторювався(/repeatplantimer)", parse_mode="HTML")
    ...
    async def repeat_plan_timer(self, message: Message):
        if await rq.check_user(message.from_user.id) is False:
            await message.answer("Щоб користуватись таймером, вкажіть город(/setcity)")
            return
        
        if await rq.check_plan_time(message.from_user.id) is False:
            await message.answer("Таймер не запущено")
            return
        
        await message.answer("Таймер буде повторюватись кожні 24 години")
        Timers.repeat = True
